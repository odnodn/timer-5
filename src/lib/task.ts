import { deepEquals, isNumber } from '@/lib/utils';
...
export const compareTasks = (a: Task, b: Task): number => {
  const as = a.sessions[a.sessions.length - 1];
  const bs = b.sessions[b.sessions.length - 1];
  if (!as && bs) {
    return -1;
  }
  if (as && !bs) {
...
export type FilterParams = {
  ...
  from?: Date;
  to?: Date;
  durationSort?: 'shortestFirst' | 'longestFirst';
};
